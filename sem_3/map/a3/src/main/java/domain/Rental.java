...
public class Rental implements Identifiable<UUID>, SerializableEntity<Rental>, Serializable {
    ...
    public String serializeToText() throws IOException {
        return id.toString() + delim +
                carToReserve.getId().toString() + delim +  // Serialize car ID
                clientName + delim +
    ...
    public Rental deserializeFromText(String line) throws IOException {
        try {
            ...
            Car carToReserve = new Car();
            carToReserve = carToReserve.deserializeFromText(carLine);

            Rental rental = new Rental(carToReserve, clientName, startDate, endDate, totalPrice);
            rental.setId(id);
