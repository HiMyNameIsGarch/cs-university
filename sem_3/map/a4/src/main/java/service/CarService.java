...
public class CarService {
    ...
    public List<Car> getAllCars() {
        Iterable<Car> iterable = carRepository.getAll();
        List<Car> list = StreamSupport.stream(iterable.spliterator(), false)
    ...
    public void initRepositoryWithData() throws RuntimeException {
        ...
        try {
            System.out.println("Loading cars from database...");
            carRepository.load();

        } catch (Exception e) {
            throw new RuntimeException("Error loading cars from db: " + e.getMessage());
        } finally {
