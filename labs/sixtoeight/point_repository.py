...
class PointRepository:
    ...
    def __distance(self, p1:Point, p2:Point):
        return ((p2.get_coord_x() - p1.get_coord_x()) ** 2) + ((p2.get_coord_y() - p1.get_coord_y()) ** 2) ** 1/2
    ...
    def get_points_inside_square(self, p:int, len:int):
        np = []
        for sp in self.__points:
            if sp.get_coord_x() <= p * len and sp.get_coord_y() <= p * len:
                np.append(sp)
        return np
    ...
    def del_points_inside_square(self, p:int, len:int):
        idx = 0
        for sp in self.__points:
            if sp.get_coord_x() <= p * len and sp.get_coord_y() <= p * len:
                del self.__points[idx]
